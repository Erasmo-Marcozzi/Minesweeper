...
def main():
    ...
    if k.lower() != 'n':
        ...
        flag = tk.PhotoImage(file='flag_win.png')
        fl = 0
        flag_placements = np.zeros_like(m, dtype=str)

        iv = tk.PhotoImage(width=40*y, height=25)
        vic = ttk.Label(root, text=str(fl)+'/'+str(b), image=iv, compound='center')
        vic.grid(row = y+1, columnspan = x)

        func = tk.PhotoImage(width=65, height=15)    
        ...
        i = tk.PhotoImage(width=35, height=35)
        i2 = tk.PhotoImage(width=41, height=41)
        for l in r:
